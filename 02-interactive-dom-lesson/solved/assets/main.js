...
function mainFunction() {
  ...
  // 
  const cardButton = document.querySelectorAll("button.card-btn");
  ...
  // 
  const userInput = document.querySelector('input.input-field');

  userInput.addEventListener("keyup", () => {
    var userInputValue = userInput.value;
    if (userInputValue.includes("h")) {
      alert("h is included!");
    }
  })
  ...
  addDiv.addEventListener('click', () => {
    var newContent = parentDiv.appendChild(document.createElement('div'));
    newContent.innerHTML = '<p>Hello!</p>';
  })
}